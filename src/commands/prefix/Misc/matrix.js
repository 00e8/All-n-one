...
module.exports = {
  name: 'matrix',
  description: 'Generate a dot matrix of an image (URL or user pfp)',
  cooldown: 5,
  aliases: ['ascii', 'dotmatrix'],
  ...
  async execute(message, args) {
    ...
    if (message.mentions.users.size > 0) {
      ...
      imageUrl = user.displayAvatarURL({ extension: 'png', size: 256 });
      ...
      if (!imageUrl.match(/^https?:\/\/.+\.(jpg|jpeg|png|gif|webp)$/i)) {
        const container = new ContainerBuilder() 
          ...
          .addTextDisplayComponents(
            new TextDisplayBuilder().setContent('Please provide a valid image URL or mention a user!\nUsage: `matrix <image_url>` or `matrix @user`')
          );
      ...
      imageUrl = message.author.displayAvatarURL({ extension: 'png', size: 256 });
    }
