...
module.exports = {
    name: 'resetxp',
    aliases: [],
    description: 'Reset a member\u2019s XP, level and streak',
    cooldown: 5,
    ...
    async execute(message, args) {
        ...
        if (!target) {
            const container = new ContainerBuilder()
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent(`-# Usage \`resetxp @user\``)
                );
            return message.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
        }

        await levelingService.resetXP(target.id, message.guild.id);

        const container = new ContainerBuilder()
            .addTextDisplayComponents(
                new TextDisplayBuilder().setContent(`-# ${emojis.success} Reset XP, level and streak for ${target}`)
            );
