...
module.exports = {
  ...
  async execute(message, args) {
    ...
    const loadingContainer = new ContainerBuilder()
      ...
      .addTextDisplayComponents(
        new TextDisplayBuilder().setContent(`-# Exporting server settings...`)
      );
    ...
    try {
      const content = DumpExporter.formatSettings(message.guild);
      ...
      const dmContainer = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
          new TextDisplayBuilder().setContent(`-# Here is the server configuration for ${message.guild.name}.`)
        )
        .addFileComponents(
