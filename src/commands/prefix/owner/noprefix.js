...
module.exports = {
    name: 'noprefix',
    description: 'Add or remove a user from the no-prefix',
    cooldown: 5,
    ...
    async execute(message, args) {
        ...
        if (action === 'add') {
            ...
            const container = new ContainerBuilder() 
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent('-# NoPrefix')
                )
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent(
                        `> - Granted to ${targetUser}\n` +
                        `> - Duration: ${durationStr}\n` +
        ...
        if (action === 'remove') {
            ...
            if (!targetUser) {
                ...
                if (record) {
                    ...
                    NoPrefix.invalidateCache(args[1]);
                    const container = new ContainerBuilder() 
                        .addTextDisplayComponents(new TextDisplayBuilder().setContent('-# NoPrefix'))
            ...
            if (!existing) {
                const container = new ContainerBuilder() 
                    .addTextDisplayComponents(new TextDisplayBuilder().setContent('-# NoPrefix'))
                    .addTextDisplayComponents(new TextDisplayBuilder().setContent(`> -# ${targetUser} is not in the no-prefix list.`))
                    .addSeparatorComponents(footer)
                    .addTextDisplayComponents(footerText);
                return message.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
            }
            ...
            const container = new ContainerBuilder() 
                ...
                .addTextDisplayComponents(
                    new TextDisplayBuilder().setContent(
                        `> - -# ${checkAgain ? 'Failed to remove' : 'Removed'} ${targetUser}\n` +
        ...
        if (action === 'list') {
            ...
            if (records.length === 0) {
                const container = new ContainerBuilder() 
                    .addTextDisplayComponents(new TextDisplayBuilder().setContent('-# NoPrefix'))
                    .addTextDisplayComponents(new TextDisplayBuilder().setContent('> No users in the noprefix list.'))
                    .addSeparatorComponents(footer)
                    .addTextDisplayComponents(footerText);
                return message.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
            }
