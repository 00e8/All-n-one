...
//
// HTML transcript generation, ported from the bot's original ticket
// system (lib/ticketUtils.js) so the ticket system doesn't need an
// extra dependency (discord-html-transcripts) just for this.
...
async function generateTranscript(ticket, categoryName, guild) {
...
<div class="messages">`;
    ...
    for (const message of allMessages) {
        ...
        if (message.content) html += `<div class="msg-content">${formatDiscordMarkdown(message.content)}</div>`;
        if (hasContainer) html += renderContainerV2(message);
