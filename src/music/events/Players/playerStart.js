  ...
  break;
      ...
      case "previous":
        ...
        if (history.length === 0) {
          const display = new TextDisplayBuilder()
            .setContent(`-# No previous track found in history.`);
          const container = new ContainerBuilder()
      ...
      case "like":
        ...
        try {
          ...
          if (alreadyLiked) {
            const display = new TextDisplayBuilder()
              .setContent(`-# ${currentLikeTrack.title} is already in your favourite list.`);
            const container = new ContainerBuilder()
              .addTextDisplayComponents(display);
            return interaction.reply({
              components: [container],
            ...
            const display = new TextDisplayBuilder()
              .setContent(`-# Added ${currentLikeTrack.title} to your favourite list.`);
            const container = new ContainerBuilder()
              .addTextDisplayComponents(display);
            return interaction.reply({
              components: [container],
          ...
          const display = new TextDisplayBuilder()
            .setContent(`-# Failed to save song to favorites. Please try again.`);
          const container = new ContainerBuilder()
            .addTextDisplayComponents(display);
          return interaction.reply({
            components: [container],
      ...
      default:
        const unknownDisplay = new TextDisplayBuilder()
          .setContent(`-# Unknown button interaction.`);
    ...
    collector.on("collect", async (interaction) => {
      try {
        if (!interaction.member?.voice?.channelId || interaction.member.voice.channelId !== player.voiceId) {
          const display = new TextDisplayBuilder()
            .setContent(`-# You must be in the same voice channel as the bot.`);
          const container = new ContainerBuilder()
            .addTextDisplayComponents(display);
          return interaction.reply({
            components: [container],
