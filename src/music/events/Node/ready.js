module.exports = {
  ...
  run: async (client, name) => {
    client.logger.log(`Music subsystem ready (Kazagumo/Shoukaku)`, "ready");
    client.logger.log(`Lavalink "${name}" is READY.`, "ready");

    const maindata = client.db.twofourseven.getAll();
    if (maindata.length > 0) {
      client.logger.log(
        `Auto Reconnect found ${maindata.length} queue${maindata.length > 1 ? "s" : ""}. Resuming all auto reconnect queue`,
        "ready"
      );
    }
