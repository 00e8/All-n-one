...
module.exports = function loadPlayerManager(client) {
  ...
  manager.on("nodeConnect", (node) => client.logger.log(`Lavalink Node "${node.name}" connected.`, "log"));
  manager.on("nodeError", (node, error) => client.logger.log(`Lavalink Node "${node.name}" error: ${error.message}`, "error"));
  manager.on("nodeDisconnect", (node, reason) => client.logger.log(`Lavalink Node "${node.name}" disconnected. Reason: ${reason || 'Unknown'}`, "warn"));

  manager.on("error", (error) => {
    if (error.message?.includes("Connection exist but player not found")) return;
    client.logger.log(`[Kazagumo] Error: ${error.message || error}`, "error");
  });

  // NOTE: shoukaku "ready" / "error" / "close" are already handled in
  // ./music/events/Node/ready.js, error.js and close.js (wired via
  // loadNodes.js). Listening again here duplicated every startup log line.
