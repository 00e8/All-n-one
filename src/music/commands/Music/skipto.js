...
module.exports = {
  ...
  async slashExecute(interaction, client) {
    ...
    if (position < 1 || position > player.queue.length) {
      const warnDisplay = new TextDisplayBuilder()
        .setContent(`-# Please provide a valid position\n-# Valid range 1-${player.queue.length}**`);
    ...
    const successDisplay = new TextDisplayBuilder().setContent(`-# Skipped to ${targetSong.title}`);
  ...
  async execute(message, args, client, prefix) {
    ...
    if (!targetSong) {
      const errorDisplay = new TextDisplayBuilder()
        .setContent(`-# Could not find song at position ${position}`);
    ...
    const successDisplay = new TextDisplayBuilder()
      .setContent(`-# Skipped to ${targetSong.title}`);
