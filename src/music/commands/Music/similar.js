...
module.exports = {
    ...
    async runSimilar(context, client, isSlash) {
        ...
        if (!player || !player.queue.current) {
            const errorDisplay = new TextDisplayBuilder().setContent(`-# No music currently playing`);
        ...
        if (tracks.length === 0) {
            const noResultsDisplay = new TextDisplayBuilder().setContent(`-# No similar songs found using \`${searchEngine}\``);
        ...
        collector.on('collect', async (i) => {
            ...
            if (i.customId === 'sim_prev') {
                ...
                await i.editReply({ components: [generateContainer(page)], flags: MessageFlags.IsComponentsV2 });
                ...
                const successDisplay = new TextDisplayBuilder()
                    .setContent(`-# Added all ${tracks.length} similar tracks to queue.`);
                const successContainer = new ContainerBuilder().addTextDisplayComponents(successDisplay);
