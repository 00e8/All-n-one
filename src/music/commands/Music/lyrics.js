...
module.exports = {
    ...
    async execute(message, args, client, prefix) {
        ...
        const loadingDisplay = new TextDisplayBuilder()
            .setContent(`-# Searching for lyrics`);
        ...
        if (!result || !result.lyrics) {
            const notFoundDisplay = new TextDisplayBuilder()
                .setContent(`-#  No lyrics found for ${track.title} by ${track.author}`);
        ...
        const infoDisplay = new TextDisplayBuilder()
            .setContent(
                `-# Artist ${track.author}\n` +
                `-# Source ${source}` +
                (hasSyncedLyrics ? `-# Sync Available` : '')
            );
        ...
        choiceCollector.on("collect", async (interaction) => {
            if (interaction.user.id !== message.author.id) {
                const errorDisplay = new TextDisplayBuilder()
                    .setContent(`-# Only ${message.author.tag} can use these buttons.`);
...
async function showStaticLyrics(client, message, track, lyrics, source, authorId) {
    ...
    collector.on("collect", async (interaction) => {
        if (interaction.user.id !== authorId) {
            const errorDisplay = new TextDisplayBuilder()
                .setContent(`-# Only the command user can use these buttons.`);
