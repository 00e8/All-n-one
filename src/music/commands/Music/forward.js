    ...
    async execute(message, args, client, prefix) {
        ...
        if (newPosition >= duration) {
            const warnDisplay = new TextDisplayBuilder()
                .setContent(
                    `-# Cannot fast forward beyond the song duration.` +
                    `-# Skipping to next song instead`
                );
        ...
        try {
            ...
            const successDisplay = new TextDisplayBuilder()
                .setContent(
                    `-# Fast forwarded ${seconds}s to ${formatTime(newPosition)}`
                );
            ...
            const errorDisplay = new TextDisplayBuilder()
                .setContent(`-# Failed to fast forward the track`);
