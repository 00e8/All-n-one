...
module.exports = {
  ...
  async slashExecute(interaction, client) {
    ...
    if (!player.queue.current) {
      const errorDisplay = new TextDisplayBuilder().setContent(`-# Play a song first.`);
    ...
    if (!time || isNaN(time)) {
      const errorDisplay = new TextDisplayBuilder().setContent(`-# Invalid time format. Examples 40, 1:30, 10s, 1m`);
      const container = new ContainerBuilder().addTextDisplayComponents(errorDisplay);
      return interaction.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
    }
    ...
    if (time <= duration) {
      ...
      const successDisplay = new TextDisplayBuilder()
        .setContent(
          `-# Successfully ${action}\n` +
          `-# Track [${song.title.substring(0, 45)}](${song.uri})\n` +
  ...
  async execute(message, args, client) {
    if (!args.length) {
      ...
      const header = new TextDisplayBuilder().setContent(`-# Seek Command`);
      const usage = new TextDisplayBuilder().setContent(`-# Usage seek time\n-# Example seek 1:30`);
  ...
  async runSeek(message, args, client) {
    ...
    if (!time || isNaN(time)) {
      const errorDisplay = new TextDisplayBuilder()
        .setContent(`-# Invalid time format. Examples 40, 1:30, 10s, 1m`);
