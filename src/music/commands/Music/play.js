...
module.exports = {
  ...
  async slashExecute(interaction, client) {
    ...
    if (!interaction.member?.voice?.channel) {
      const errorDisplay = new TextDisplayBuilder()
        .setContent(`-# You need to be in a voice channel first.`);
    ...
    if (!interaction.guild.members.me.permissions.has([
      ...
      const errorDisplay = new TextDisplayBuilder()
        .setContent(`-# I don't have enough permissions Please give me \`CONNECT\` and \`SPEAK\`.`);
    ...
    try {
      ...
      if (!hasAvailableNodes(client.manager)) {
        const errorDisplay = new TextDisplayBuilder()
          .setContent(`-# The music server is currently unavailable. Please try again later.`);
      ...
      // Guard against a leftover Discord voice session from a very recent stop/disconnect:
      // Kazagumo may already think there is no player while Discord still shows us as
      // connected. Creating a new player on top of that half-closed session is what causes
      // "play" to silently do nothing until something forces a full leave+rejoin.
      ...
      if (!player) {
        ...
        if (player.voiceId !== channel.id) {
          const errorDisplay = new TextDisplayBuilder()
            .setContent(`-# I'm already connected to a different voice channel.`);
      ...
      if (!searchResult || !searchResult.tracks || !searchResult.tracks.length) {
        const errorDisplay = new TextDisplayBuilder()
          .setContent(`> -#   **No results found for "${query}"**`);
      ...
      if (searchResult.type === "PLAYLIST") {
        ...
        const successDisplay = new TextDisplayBuilder()
          .setContent(`-# Playlist Added\n-# ${searchResult.tracks.length} tracks from ${searchResult.playlistName}`);
      ...
      const infoDisplay = new TextDisplayBuilder()
        .setContent(
          `-# Playing [${truncateTitle(track.title, 25)}](${track.uri})\n-# by ${cleanAuthorName(track.author)}\n` +
          `-# Length ${convertTime(track.length)}`
        );
          ...
          if (action === 'remove') {
            try {
              ...
              if (trackIndex !== -1) {
                ...
                const updatedDisplay = new TextDisplayBuilder()
                  .setContent(`-# Removed [${truncateTitle(removedTrack.title, 25)}](${removedTrack.uri}) from queue.`);
            ...
            try {
              ...
              if (trackIndex !== -1) {
                ...
                const updatedDisplay = new TextDisplayBuilder()
                  .setContent(`-# Moved [${truncateTitle(trackToMove.title, 25)}](${trackToMove.uri}) to next in queue.`);
  ...
  async execute(message, args, client, prefix) {
    ...
    try {
      ...
      // Same leftover-session guard as the slash path: if Discord still shows us connected
      // (e.g. right after "stop") even though Kazagumo has no player anymore, force a real
      // disconnect before creating a fresh player, otherwise play() can go silent.
      if (!player && message.guild.members.me?.voice?.channelId) {
        try {
      ...
      if (!searchResult || !searchResult.tracks || !searchResult.tracks.length) {
        const errorDisplay = new TextDisplayBuilder()
          .setContent(`-# No results found for that song`);
      ...
      if (addedTracks.length === 0) {
        const errorDisplay = new TextDisplayBuilder()
          .setContent(`-# No tracks could be processed`);
      ...
      if (searchResult.type === "PLAYLIST") {
        const successDisplay = new TextDisplayBuilder()
          .setContent(`-# Playlist Added\n-# ${addedTracks.length} tracks from ${searchResult.playlistName}`);
        ...
        const infoDisplay = new TextDisplayBuilder()
          .setContent(
            `> -# Playing [${truncateTitle(track.track.title, 25)}](${track.track.uri})\n> -# by ${cleanAuthorName(track.track.author)}\n` +
            `> -# Length ${convertTime(track.track.length)}`
          );
        ...
        if (track.position > 0 && replyMsg) {
          ...
          collector.on('collect', async (interaction) => {
            if (!interaction.member.voice.channel || interaction.member.voice.channel.id !== player.voiceId) {
              return interaction.reply({ content: `-# You must be in my voice channel to use this.`, ephemeral: true });
            }
