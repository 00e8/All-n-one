...
module.exports = {
    ...
    async slashExecute(interaction, client) {
        ...
        try {
            ...
            const successDisplay = new TextDisplayBuilder()
                .setContent(`-# Rewound ${seconds}s to ${formatTime(newPosition)}`);
            const container = new ContainerBuilder().addTextDisplayComponents(successDisplay);
            return interaction.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
        } catch (error) {
            console.error("Error rewinding:", error);
            const errorDisplay = new TextDisplayBuilder().setContent(`-# Failed to rewind the track.`);
            const container = new ContainerBuilder().addTextDisplayComponents(errorDisplay);
            return interaction.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
        }
    },
    ...
    async execute(message, args, client, prefix) {
        ...
        if (args.length > 0) {
            ...
            if (isNaN(seconds) || seconds <= 0) {
                const usageDisplay = new TextDisplayBuilder()
                    .setContent(
                        `-# Usage ${prefix}rewind [seconds]*\n` +
                        `-# Example ${prefix}rewind 30 - Rewind 30 seconds`
                    );
        ...
        try {
            ...
            const errorDisplay = new TextDisplayBuilder()
                .setContent(`-# Failed to rewind the track.`);
