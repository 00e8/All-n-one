...
module.exports = {
    ...
    async slashExecute(interaction, client) {
        ...
        if (!player.queue.current) {
            const errorDisplay = new TextDisplayBuilder().setContent(`-# No music playing.`);
        ...
        if (input === "cancel") {
            if (!existingTimer) {
                const errorDisplay = new TextDisplayBuilder().setContent(`-# No active timer.`);
                const container = new ContainerBuilder().addTextDisplayComponents(errorDisplay);
                return interaction.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
            }
            clearTimeout(existingTimer.timeout);
            player.data.delete("sleepTimer");
            const successDisplay = new TextDisplayBuilder().setContent(`-# Timer cancelled.`);
        ...
        if (!duration || duration < 1 || duration > 180) {
            const errorDisplay = new TextDisplayBuilder().setContent(`-# Invalid duration\n-# Use30m, 1h, 45m (1-180 min)`);
            const container = new ContainerBuilder().addTextDisplayComponents(errorDisplay);
            return interaction.reply({ components: [container], flags: MessageFlags.IsComponentsV2 });
        }

        const endTime = Date.now() + (duration * 60 * 1000);
        const headerDisplay = new TextDisplayBuilder().setContent(`-# Timer Set`);
    ...
    async execute(message, args, client, prefix) {
        ...
        const infoDisplay = new TextDisplayBuilder()
            .setContent(
                `-# Duration ${duration}m\n` +
                `-# Ends <t:${Math.floor(endTime / 1000)}:t>\n` +
                `-# Action Disconnect from VC\n\n` +
                `-# Cancel ${prefix}sleep cancel`
            );
                ...
                if (member && member.voice.channel) {
                    ...
                    if (textChannel) {
                        const sleepDisplay = new TextDisplayBuilder()
                            .setContent(`-# Timer ended - ${message.author} disconnected.`);
