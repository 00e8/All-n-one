...
module.exports = {
    ...
    async runSearch(context, client, query, isSlash) {
        ...
        if (!node) {
            const errorDisplay = new TextDisplayBuilder().setContent(`-# No music node available`);
        ...
        if (songs.length === 0) {
            const noResultsDisplay = new TextDisplayBuilder().setContent(`> -#   **No results found for \`${query}\`**`);
        ...
        const generateContainer = (p) => {
            ...
            // Header buttons (Songs, Artists, Close)
            const headerRow = new ActionRowBuilder().addComponents(
                new ButtonBuilder()
                    .setCustomId('mode_songs')
                    .setLabel('Songs')
                    .setStyle(mode === 'songs' ? ButtonStyle.Secondary : ButtonStyle.Secondary),
                new ButtonBuilder()
                    .setCustomId('mode_artists')
                    .setLabel('Artists')
                    .setStyle(mode === 'artists' ? ButtonStyle.Secondary : ButtonStyle.Secondary),
                new ButtonBuilder()
        ...
        mainCollector.on('collect', async (i) => {
            ...
            if (i.customId === 'mode_songs') {
                ...
                const successDisplay = new TextDisplayBuilder()
                    .setContent(`-# Added [${track.title}](${track.uri}) to queue.`);
                ...
                const successDisplay = new TextDisplayBuilder()
                    .setContent(`-# Added all ${songs.length} tracks to queue.`);
                const container = new ContainerBuilder().addTextDisplayComponents(successDisplay);
                ...
                try {
                    ...
                    if (!artist) {
                        return i.editReply({ content: `-# Artist data not found. Please try searching again.` });
                    }

                    const artistTracks = await loadArtistTracks(artist.author || artist.title);

                    if (!artistTracks || artistTracks.length === 0) {
                        return i.editReply({ content: `-# No tracks found for this artist.` });
                    }
                    ...
                    const generateArtistContainer = (ap) => {
                        ...
                        const artistNavRow = new ActionRowBuilder().addComponents(
                        ...
                        container.addActionRowComponents(artistNavRow);

                        return container;
                    };

                    const artistMsg = await i.editReply({
                        components: [generateArtistContainer(0)],
                        flags: MessageFlags.IsComponentsV2
                    });
                    ...
                    artistCollector.on('collect', async (ai) => {
                        if (ai.customId === 'aprev') {
                            ...
                            const successDisplay = new TextDisplayBuilder()
                                .setContent(`-# Added all ${artistTracks.length} tracks to queue.`);
                            const container = new ContainerBuilder().addTextDisplayComponents(successDisplay);
                        ...
                        await ai.update({ components: [generateArtistContainer(artistPage)], flags: MessageFlags.IsComponentsV2 });
                    });
                } catch (error) {
                    console.error('Error in artist view:', error);
                    return i.editReply({ content: `-# Something went wrong while loading artist tracks.` }).catch(() => { });
                }
            }
        });
