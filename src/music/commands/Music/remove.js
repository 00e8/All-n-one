  ...
  async execute(message, args, client, prefix) {
    ...
    if (args[0]) {
      ...
      if (isNaN(position) || position < 0 || position >= player.queue.length) {
        const errorDisplay = new TextDisplayBuilder()
          .setContent(
            `-# Invalid position ${args[0]}\n` +
            `-# Total songs in queue ${player.queue.length}`
          );
      ...
      const successDisplay = new TextDisplayBuilder()
        .setContent(`-# Removed [${song.title}](${song.uri})`);
    ...
    const collector = msg.createMessageComponentCollector({
      filter: (i) => {
        ...
        const errorDisplay = new TextDisplayBuilder()
          .setContent(`-# Only ${message.author.tag} can use this`);
    ...
    collector.on('collect', async (interaction) => {
      if (player.queue.length === 0 && interaction.customId !== 'clear_queue') {
        const errorDisplay = new TextDisplayBuilder()
          .setContent(`-# he queue is now empty.`);
      ...
      if (interaction.customId === 'select_track') {
        ...
        if (player.queue.length === 0) {
          const emptyDisplay = new TextDisplayBuilder()
            .setContent(`-# Removed ${removedTracks.length} track(s). The queue is now empty`);
          ...
          const successDisplay = new TextDisplayBuilder()
            .setContent(`-# Removed ${removedTracks.length} track(s)`);
        ...
        const successDisplay = new TextDisplayBuilder()
          .setContent(`-# Cleared ${queueSize} tracks from the queue`);
    ...
    collector.on('end', async (collected, reason) => {
      if (reason === 'idle' || reason === 'time') {
        const timeoutDisplay = new TextDisplayBuilder()
          .setContent(`-# Session timed out. Use the command again if needed.`);
