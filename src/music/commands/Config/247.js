...
module.exports = {
  ...
  async slashExecute(interaction, client) {
    ...
    const isOwner = client.owners?.includes(interaction.user.id) || false;
    if (!interaction.member.permissions.has(PermissionFlagsBits.Administrator) && !isOwner) {
      const errorDisplay = new TextDisplayBuilder()
        .setContent(`-# You need Administrator permissions`);
  ...
  async execute(message, args, client, prefix) {
    const isOwner = client.owners?.includes(message.author.id) || false;
    if (!message.member.permissions.has(PermissionFlagsBits.Administrator) && !isOwner) {
      const errorDisplay = new TextDisplayBuilder()
        .setContent(`-#   **You need **Administrator** permissions`);
      const container = new ContainerBuilder().addTextDisplayComponents(errorDisplay);
    ...
    try {
      ...
      if (!voiceChannel) {
        ...
        const infoDisplay = new TextDisplayBuilder()
          .setContent(`-# You need to be in a voice channel `);
      ...
      if (player && player.voiceId) {
        if (voiceChannel.id !== player.voiceId) {
          ...
          const infoDisplay = new TextDisplayBuilder()
            .setContent(`-# You need to be in the same voice channel as me `);
