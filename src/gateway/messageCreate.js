...
// Drop stale cache entries the instant the underlying setting changes
// (e.g. via a command), instead of waiting up to CACHE_TTL for them to expire.
cacheBus.on('invalidate:GuildConfig', ({ guildId }) => {
  if (guildId) guildConfigCache.delete(guildId);
});
cacheBus.on('invalidate:AutoReact', ({ guildId }) => {
  if (guildId) autoReactCache.delete(guildId);
});
cacheBus.on('invalidate:AFK', ({ guildId, userId }) => {
  if (!guildId || !userId) return;
  const key = `${guildId}:${userId}`;
  afkMentionCache.delete(key);
  afkAuthorCache.delete(key);
});
cacheBus.on('invalidate:Blacklist', ({ type, entityId }) => {
  if (!entityId) return;
  if (type === 'user') blacklistUserCache.delete(entityId);
  if (type === 'guild') blacklistGuildCache.delete(entityId);
});
cacheBus.on('invalidate:mediaChannel', ({ guildId }) => {
  if (guildId) mediaChannelCache.delete(guildId);
});
cacheBus.on('invalidate:aiChannel', ({ guildId, channelId }) => {
  if (guildId && channelId) aiChannelCache.delete(`${guildId}:${channelId}`);
  else if (guildId) {
    for (const key of aiChannelCache.keys()) {
      if (key.startsWith(`${guildId}:`)) aiChannelCache.delete(key);
    }
  }
});
...
module.exports = {
  ...
  async execute(message, client) {
      ...
      if (!(inAiChannel && !usedPrefixFlag && !isAiCommand && !hasNoPrefix)) {
        try {
          ...
          if (message.author.id !== config.OWNER_ID) {
            ...
            const { onCooldown, remaining } = checkCooldown(message.author.id, command.name, cooldownMs);
            if (onCooldown) {
    ...
    if (userAFK) {
      ...
      const afkRemovedContainer = new ContainerBuilder() 
        .addTextDisplayComponents(
    ...
    if (message.mentions.users.size > 0) {
      for (const [userId, user] of message.mentions.users) {
        ...
        if (mentionedAFK) {
          ...
          const afkNoticeContainer = new ContainerBuilder() 
            .addTextDisplayComponents(
