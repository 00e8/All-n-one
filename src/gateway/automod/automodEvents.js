...
// Apply `/automod` changes immediately instead of waiting up to
// AUTOMOD_CACHE_TTL / WHITELIST_CACHE_TTL for the stale cache to expire.
cacheBus.on('invalidate:AutomodConfig', ({ guildId }) => {
    if (guildId) automodConfigCache.delete(guildId);
});
cacheBus.on('invalidate:AutomodWhitelist', ({ guildId, targetId }) => {
    if (guildId && targetId) whitelistCache.delete(`${guildId}:${targetId}`);
});
