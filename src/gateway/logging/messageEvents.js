...
// Apply `/logging` changes immediately instead of waiting up to
// LOGGING_CACHE_TTL for the stale cached config to expire.
cacheBus.on('invalidate:LoggingConfig', ({ guildId }) => {
    if (guildId) loggingConfigCache.delete(guildId);
});
