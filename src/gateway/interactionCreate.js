...
function disableAllButtons(rawComponents) {
  return rawComponents.map(comp => {
    ...
    if (Array.isArray(c.components)) {
      c.components = disableAllButtons(c.components);
    }
    return c;
  });
}
...
// Apply `/ignore` add/remove changes immediately instead of waiting up
// to ICACHE_TTL for the stale cached value to expire.
cacheBus.on('invalidate:bypassUser', ({ guildId, userId }) => {
  iCache.delete(`bypass:${guildId}:${userId}`);
});
cacheBus.on('invalidate:ignoredUser', ({ guildId, userId }) => {
  iCache.delete(`ignoredUser:${guildId}:${userId}`);
});
cacheBus.on('invalidate:ignoredChannel', ({ guildId, channelId }) => {
  iCache.delete(`ignoredChannel:${guildId}:${channelId}`);
});
cacheBus.on('invalidate:ignoredCommand', ({ guildId, commandName }) => {
  iCache.delete(`ignoredCmd:${guildId}:${commandName}`);
});
cacheBus.on('invalidate:commandLock', ({ commandName }) => {
  iCache.delete(`cmdLock:${commandName}`);
});
  ...
  async execute(interaction) {
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.user.id !== config.OWNER_ID) {
        ...
        const { onCooldown, remaining } = checkCooldown(interaction.user.id, interaction.commandName, cooldownMs);
        if (onCooldown) {
        ...
        if (parts.length === 2) {
          ...
          if (ids.length === 2 && roleplayActions[action]) {
            ...
            try {
              ...
              const disabledComponents = disableAllButtons(rawComponents);
              await interaction.message.edit({
                components: disabledComponents,
                flags: MessageFlags.IsPersistent | MessageFlags.IsComponentsV2
              }).catch(() => {});
            } catch (disableError) {
              console.error(`Error disabling roleplay button ${action}:`, disableError);
            }
