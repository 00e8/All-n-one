...
(async () => {
  ...
  try {
    printLoading('Music subsystem');
    const musicConfig = require('./music/config.js');
    client.config = musicConfig;
    client.owners = [config.OWNER_ID];
    client.prefix = config.PREFIX;
    client.emoji = require('./music/emojis.js');
    client.logger = require('./music/utils/logger.js');
    client.db = require('./music/structures/Database.js');

    require('./music/loaders/loadPlayerManager.js')(client);
    require('./music/loaders/loadNodes.js')(client);
    require('./music/loaders/loadPlayers.js')(client);

const VoiceHealthMonitor = require('./music/utils/voiceHealthMonitor.js');
client.voiceHealthMonitor = new VoiceHealthMonitor(client);

    // Wait for Lavalink to actually come online (or time out) before moving
    // on, so the "Music subsystem ready" / "Lavalink is READY" lines print
    // right here — grouped under the music section — instead of showing up
    // later, out of order, once the node happens to connect.
    await new Promise((resolve) => {
      const timeout = setTimeout(resolve, 15000);
      client.manager.shoukaku.once('ready', () => {
        clearTimeout(timeout);
        resolve();
      });
    });
  } catch (error) {
    printError(`Music subsystem failed to initialize: ${error.message}`);
  }
  ...
  const dbModules = {
    ...
    socialDownloader: require('./data/socialDownloader'),
    leveling:    require('./data/leveling'),
  };
  ...
  client.once('clientReady', async () => {
    ...
    try {
      ...
      printError('Failed to initialize database-dependent: ' + error.message);
    }
