...
const setMediaChannel = async (guildId, channelId) => {
    const result = await run('INSERT INTO media_channels (guild_id, channel_id) VALUES ($1, $2) ON CONFLICT (guild_id) DO UPDATE SET channel_id = $2', [guildId, channelId]);
    cacheBus.emit('invalidate:mediaChannel', { guildId });
    return result;
};
const getMediaChannel = (guildId) =>
    getOne('SELECT channel_id FROM media_channels WHERE guild_id = $1', [guildId]);
const removeMediaChannel = async (guildId) => {
    const result = await run('DELETE FROM media_channels WHERE guild_id = $1', [guildId]);
    cacheBus.emit('invalidate:mediaChannel', { guildId });
    return result;
};
