...
const categories = {
    ...
    tickets: {
        ...
        commands: [
            'ticket panel', 'ticket settings', 'ticket add', 'ticket remove',
            'ticket close', 'ticket blacklist add', 'ticket blacklist remove',
            'ticket blacklist list', 'ticket help'
        ]
    },
    welcome: {
        name: 'Welcome',
    ...
    social: {
        ...
        catEmoji: 'cat_social',
        commands: ['youtube', 'github', 'wikipedia', 'news', 'google', 'ping', 'crypto balance', 'crypto price', 'crypto convert', 'crypto transaction', 'crypto news', 'crypto gainers', 'crypto losers', 'autodl enable', 'autodl disable', 'autodl status']
    },
    utility: {
        name: 'Utility',
    ...
    music: {
        name: 'Music',
        description: 'Play, queue & control music playback',
        catEmoji: 'music',
        commands: [
            'play', 'join', 'leave', 'leavecleanup', 'pause', 'resume', 'stop',
            'skip', 'forceskip', 'skipto', 'previous', 'replay', 'seek', 'rewind', 'forward',
            'volume', 'loop', 'shuffle', 'autoplay', 'sleeptimer',
            'queue', 'clearqueue', 'remove', 'move', 'history', 'grab',
            'nowplaying', 'search', 'lyrics', 'similar', 'artistradio', 'mood',
            '247', 'source', 'forcefix'
        ]
    },
};
...
const extraCategories = {
    ...
    leveling: {
        name: 'Leveling',
        description: 'XP, ranks, streaks, rewards & leaderboards',
        catEmoji: 'cat_tracking',
        commands: [
            'leveling', 'rank', 'xptop', 'streak', 'rewards', 'claimreward',
            'givexp', 'setlevel', 'resetxp',
            'levelsetup', 'xpconfig', 'xprange', 'xpcooldown', 'voicexp', 'multiplier', 'xpblacklist',
            'serverstats'
        ]
    },
    music: {
        name: 'Music',
        description: 'Play, queue, and control music playback',
        catEmoji: 'music',
        commands: [
            'play', 'search', 'join', 'leave', 'leavecleanup',
            'pause', 'resume', 'stop', 'skip', 'forceskip', 'skipto', 'previous',
            'queue', 'clearqueue', 'remove', 'move', 'shuffle', 'loop', 'autoplay',
            'seek', 'rewind', 'forward', 'replay', 'volume',
            'nowplaying', 'lyrics', 'history', 'grab', 'source',
            'artistradio', 'mood', 'similar', 'sleeptimer', 'forcefix'
        ]
    },
};
...
async function createHelpContainer(categoryKey = null, botAvatarURL, userAvatarURL, clientId, guildId = null, botUsername = 'Bot') {
    ...
    if (!categoryKey) {
        ...
        const totalCommands = Object.values(allCats).reduce(
            (sum, cat) => sum + (cat.commands ? cat.commands.length : 0),
            0
        );
        ...
        const headerSection = new SectionBuilder()
            ...
            .addTextDisplayComponents(
                new TextDisplayBuilder().setContent(
                    ...
                    `-# - Join [Support](${config.SUPPORT_SERVER}) For Help`
                )
            );
        if (botAvatarURL) {
        ...
        if (categoryKey === 'music' && category.subcategories) {
            ...
            const commandsSection = new SectionBuilder();
            const commandList = category.commands.map((cmd, index) => `-# **${index + 1}.** \`${cmd}\``).join('\n');
            commandsSection.addTextDisplayComponents(
...
module.exports = {
    ...
    async execute(interactionOrMessage, args = []) {
        ...
        container
            .addSeparatorComponents(new SeparatorBuilder().setSpacing(SeparatorSpacingSize.Small).setDivider(true))
            .addTextDisplayComponents(
                new TextDisplayBuilder().setContent('-# Powered By HYZEX Dev')
            );
