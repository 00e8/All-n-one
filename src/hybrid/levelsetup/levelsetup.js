...
const DEFAULT_MSG = '{mention} leveled up to **Level {level}** \u{1F389}';
...
async function generatePreviewCard(author, backgroundUrl) {
    ...
    try {
        ...
        if (!bg) {
            const bgPath = path.join(__dirname, '..', '..', 'assets', 'levelbg.png');
...
async function buildPayload(guild, author, step, state, status = 'idle') {
    const isPreviewStep = (step === 'image' || step === 'preview') && status === 'idle';

    if (isPreviewStep) {
        try {
            const attachment = await generatePreviewCard(author, state.level_up_background);
            return {
                components: [await buildWizardPanel(guild, step, state, status, 'attachment://preview.png')],
                files: [attachment],
    ...
    return {
        components: [await buildWizardPanel(guild, step, state, status)],
        flags: MessageFlags.IsComponentsV2,
    };
}
...
function getSystemThumbnail(guild, client) {
    const guildIcon = guild?.iconURL?.({ size: 128, extension: 'png' });
    if (guildIcon) return guildIcon;
    return client.user?.displayAvatarURL?.({ size: 128, extension: 'png' }) ?? null;
}
...
async function buildWizardPanel(guild, step, state, status = 'idle', previewUrl = null) {
    const container = new ContainerBuilder() 
    const thumbUrl = getSystemThumbnail(guild, guild.client);
    ...
    if (step === 'welcome') {
        addHeader(
            container,
            thumbUrl,
            `> - -# Leveling UP Setup Wizard\n> - -# Welcome Let's get things configured.\n> - -# Use this interactive wizard to set up where and how levelup notifications appear.`
        );
        rows.push(
        ...
        if (previewUrl) {
            ...
            container.addTextDisplayComponents(
                new TextDisplayBuilder().setContent(`-# Currently using: Default\n-# Upload an image below to customize it (661x254 recommended)`)
            );
        }
        ...
        const msgDisplay = state.level_up_message.length > 50
            ...
            : state.level_up_message;
        const rewards = (await lvl.listRewards(guild.id)).length;
        container.addSeparatorComponents(
            new SeparatorBuilder().setDivider(true).setSpacing(SeparatorSpacingSize.Small)
        );
        container.addTextDisplayComponents(
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('levelsetup')
        .setDescription('Step-by-step setup wizard for the leveling')
        .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild),

    name: 'levelsetup',
    aliases: ['lvlsetup', 'levelconfigure'],
    description: 'Step-by-step setup wizard for the leveling',
    cooldown: 5,
    ...
    async execute(interactionOrMessage) {
        ...
        if (!hasAny) {
            return interactionOrMessage.reply({
                content: `-# ${emojis.error} You need **Manage Server** or **Manage Roles** to use this command.`,
        ...
        await lvl.ensureGuildSettings(guild.id);
        const dbSettings = (await lvl.getGuildSettings(guild.id)) ?? {};
        ...
        const sent = await channel.send({
            components: [await buildWizardPanel(guild, currentStep, wizardState)],
            flags: MessageFlags.IsComponentsV2,
        });
        ...
        collector.on('collect', async (i) => {
            if (i.user.id !== author.id) {
                try {
                    await i.reply({
                        content: `-# ${emojis.info} Only the user who ran this command can use these buttons.`,
                        flags: MessageFlags.Ephemeral,
                    });
                } catch {}
                return;
            ...
            if (
                ...
                return i.reply({
                    content: `${emojis.error} You need **Manage Server** to edit core configurations.`,
                    flags: MessageFlags.Ephemeral,
                });
            }
            ...
            if (i.customId === 'wiz_cancel') {
                try {
                    await i.update({
                        components: [await buildWizardPanel(guild, 'welcome', null, 'closed')],
                        flags: MessageFlags.IsComponentsV2,
                    });
                } catch {}
                ...
                if (ms) {
                    ...
                    if (isDiscordCdnUrl(url)) {
                        ...
                        return sent.edit(await buildPayload(guild, author, currentStep, wizardState)).catch(() => {});
                    }
                ...
                await lvl.patchGuildSettings(guild.id, 'enabled', 1);
                await lvl.patchGuildSettings(guild.id, 'level_up_enabled', wizardState.level_up_enabled ? 1 : 0);
                await lvl.patchGuildSettings(guild.id, 'level_up_channel', wizardState.level_up_channel);
                await lvl.patchGuildSettings(guild.id, 'level_up_dm', wizardState.level_up_dm ? 1 : 0);
                await lvl.patchGuildSettings(guild.id, 'level_up_message', wizardState.level_up_message);
                await lvl.patchGuildSettings(guild.id, 'level_up_background', wizardState.level_up_background);

                try {
                    await i.update({
                        components: [await buildWizardPanel(guild, currentStep, null, 'saved')],
                        flags: MessageFlags.IsComponentsV2,
                    });
                } catch (e) {
                ...
                if (!isStaff) return i.reply({ content: `${emojis.error} You need **Manage Roles**.`, flags: MessageFlags.Ephemeral });
                ...
                roleCol.on('collect', async (ri) => {
                    ...
                    if (!role || !role.editable) {
                        return ri.reply({ content: `${emojis.error} Role not manageable by bot.`, flags: MessageFlags.Ephemeral });
                    }
                    ...
                    const level = parseInt(rm.fields.getTextInputValue('l_input'), 10);
                    if (!level || level < 1 || level > 1000) return rm.reply({ content: 'Invalid level.', flags: MessageFlags.Ephemeral });

                    await lvl.addReward(guild.id, level, role.id);
                    await rm.deferUpdate();
                    await i.deleteReply().catch(() => {});
                    await sent.edit(await buildPayload(guild, author, currentStep, wizardState)).catch(() => {});
                });
                return;
            }
                ...
                const allRewards = await lvl.listRewards(guild.id);
                if (!allRewards.length) return i.reply({ content: `${emojis.error} No rewards configured.`, flags: MessageFlags.Ephemeral });
                ...
                rmCol.on('collect', async (ri) => {
                    ...
                    await lvl.removeReward(guild.id, parseInt(lstr, 10), rId);
                    await i.deleteReply().catch(() => {});
                    await sent.edit(await buildPayload(guild, author, currentStep, wizardState)).catch(() => {});
                });
                return;
            ...
            try {
                const payload = await buildPayload(guild, author, currentStep, wizardState);
                ...
                const errPayload = {
                    content: `-# ${emojis.error} Setup panel error \`${err.message}\``,
                    flags: MessageFlags.Ephemeral,
                };
                if (i.replied || i.deferred) {
        ...
        collector.on('end', async (_, reason) => {
            ...
            const status = reason === 'time' ? 'expired' : 'closed';
            sent
                .edit({
                    components: [await buildWizardPanel(guild, currentStep, null, status)],
                    flags: MessageFlags.IsComponentsV2,
                })
                .catch(() => {});
        });
