...
module.exports = {
    async execute(interactionOrMessage, args = []) {
        ...
        if (!botPerms.has(PermissionFlagsBits.ManageChannels)) {
            return reply(interactionOrMessage, 'Missing Permissions', '-# I need the Manage Channels permission to unlock voice channels.');
        }
        ...
        try {
            ...
            return reply(interactionOrMessage, 'Voice Unlock', `-# Unlocked ${channel.name}. Users can now join.`);
        } catch (error) {
            ...
            return reply(interactionOrMessage, 'Error', `-# Failed to unlock channel: ${error.message}`);
        }
    }
};
