...
module.exports = {
    async execute(interactionOrMessage, args = []) {
        ...
        if (!botPerms.has(PermissionFlagsBits.ManageChannels)) {
            return reply(interactionOrMessage, 'Missing Permissions', '-# I need the Manage Channels permission to lock voice channels.');
        }
        ...
        try {
            ...
            return reply(interactionOrMessage, 'Voice Lock', `-# Locked ${channel.name} New users cannot join.`);
        } catch (error) {
            ...
            return reply(interactionOrMessage, 'Error', `Failed to lock channel: ${error.message}`);
        }
    }
};
